...
class Network:
    ...
    FIRST_KNOWN_NODE_HOSTNAME = "192.168.1.175:5000"
    ...
    def other_nodes_exist(self) -> bool:
        if len(self.known_nodes_memory.known_nodes) == 0:
            ...
            len(self.known_nodes_memory.known_nodes) == 1
            and self.known_nodes_memory.known_nodes[0].hostname == self.node.hostname
        ):
            return False
        else:
            return True
    ...
    def join_network(self):
        ...
        if self.other_nodes_exist:
            ...
            if default_node_answered:
                ...
                logging.info(
                    "Default node didn't answer. This could be caused by a network issue."
                )
